...
app.post("/signup", async (req, res) => {
  try {
    ...
    res.json({ message: "User created", user: registerUser });
  } catch (err) {
    console.log(err);
...
app.post("/login", async (req, res) => {
  try {
    ...
    const loginUsers = new User({ username });
    const loginedUser = await User.register(loginUsers, password);
    res.json({ message: "User Login", user: loginedUser });
  } catch (err) {
    console.log(err);
    res.status(400).json({ error: err.message });
  }
